...
<body style="margin: 0px auto;">
	<div class="k-line-select">
		<div class="fr out">
			<div id="chart_time" style="float: left;">
				<a class="item" id="time_d" data-time="1440"><?php echo L('trade.rixian');?></a>
				<a class="item" id="time_6h" data-time="360"><?php echo L('trade.6xiaoshixian');?></a>
				<a class="item" id="time_1h" data-time="60"><?php echo L('trade.1xiaoshixian');?></a>
				<a class="item" id="time_30m" data-time="30"><?php echo L('trade.30fenzhongxian');?></a>
				<a class="item" id="chart_15m" data-time="15"><?php echo L('trade.15fenzhongxian');?></a>
				<a class="item" id="chart_5m" data-time="5"><?php echo L('trade.5fenzhongxian');?></a>
			</div>
		</div>
	</div>
	...
	<script>
			...
			function paintChart(){
				...
				Highcharts.setOptions({global:{useUTC:false},lang:{months:['<?php echo L('trade.yiyue');?>','<?php echo L('trade.eryue');?>','<?php echo L('trade.sanyue');?>','<?php echo L('trade.siyue');?>','<?php echo L('trade.wuyue');?>','<?php echo L('trade.liuyue');?>','<?php echo L('trade.qiyue');?>','<?php echo L('trade.bayue');?>','<?php echo L('trade.jiuyue');?>','<?php echo L('trade.shiyue');?>','<?php echo L('trade.shiyiyue');?>','<?php echo L('trade.shieryue');?>'],shortMonths:['<?php echo L('trade.yiyue');?>','<?php echo L('trade.eryue');?>','<?php echo L('trade.sanyue');?>','<?php echo L('trade.siyue');?>','<?php echo L('trade.wuyue');?>','<?php echo L('trade.liuyue');?>','<?php echo L('trade.qiyue');?>','<?php echo L('trade.bayue');?>','<?php echo L('trade.jiuyue');?>','<?php echo L('trade.shiyue');?>','<?php echo L('trade.shiyiyue');?>','<?php echo L('trade.shieryue');?>'],weekdays:['<?php echo L('trade.zhouri');?>','<?php echo L('trade.zhouyi');?>','<?php echo L('trade.zhouer');?>','<?php echo L('trade.zhousan');?>','<?php echo L('trade.zhousi');?>','<?php echo L('trade.zhouwu');?>','<?php echo L('trade.zhouliu');?>'],thousandsSep:'',rangeSelectorZoom:"<?php echo L('trade.fanwei');?>"},rangeSelector:{buttons:[{type:'minute',count:60,text:'1h'},{type:'minute',count:120,text:'2h'},{type:'minute',count:360,text:'6h'},{type:'minute',count:720,text:'12h'},{type:'day',count:1,text:'1d'},{type:'week',count:1,text:'1w'},{type:'all',text:'<?php echo L('trade.quanbu');?>'}],selected:6,inputEnabled:false},credits:{enabled:false},});
				chartData.callee.chart=new Highcharts.StockChart({chart:{renderTo:'paint_chart',animation:Highcharts.svg},title:{text:""},yAxis:[{labels:{style:{color:'#e55600'},x:15,align:'left'},title:{text:'<?php echo L('trade.jiage');?> ',style:{color:'#e55600'},margin:-10},offset:0,height:window_height/100*60,opposite:false,lineWidth:2,gridLineDashStyle:'Dash',showLastLabel:true},{labels:{style:{color:'#4572A7'},x:15,align:'left'},title:{text:'<?php echo L('trade.chengjiaoliang');?> ',style:{color:'#4572A7'},margin:-10},offset:0,top:window_height/100*70,height:window_height/100*20,opposite:false,lineWidth:2,gridLineDashStyle:'Dash',showLastLabel:true}],xAxis:{type:'datetime'},plotOptions:{candlestick:{color:'#f01717',upColor:'#0ab92b'}},
					tooltip:{crosshairs:[true,true],shared:true,xDateFormat:'%Y-%m-%d %H:%M %A',color:'#f0f',borderColor:'#058dc7'},series:[{animation:false,name:'<?php echo L('trade.jiage');?>',type:'candlestick',data:[]},{animation:false,name:'<?php echo L('trade.chengjiaoliang');?>',type:'column',color:'#4572A7',yAxis:1,data:[],tooltip:{valueDecimals:4}},{animation:false,name:'<?php echo L('trade.05junxian');?>',type:'spline',color:'#FF00FF',data:[],threshold:null,lineWidth:1,tooltip:{valueDecimals:6}},{animation:false,name:'<?php echo L('trade.20junxian');?>',type:'spline',color:'#450fff',data:[],threshold:null,lineWidth:1,tooltip:{valueDecimals:6}}]});
			}
