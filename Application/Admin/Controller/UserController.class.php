	...
	public function index($name = NULL, $field = NULL, $status = NULL)
	{
		...
		foreach ($list as $k => $v) {
			...
			$list[$k]['invit_3'] = M('User')->where(array('id' => $v['invit_3']))->getField('username');
                        ...
                        //依次获取左右区的业绩
                        $yeji = $this ->mywd_nextlevel_json($v['id']);
                        $list[$k]['left_yeji'] = $yeji['left_yeji'];
                        $list[$k]['right_yeji'] = $yeji['right_yeji'];
		}
		...
		$this->display();
         ...
         //分别获取不同层级的元素的
        public function mywd_nextlevel($id,$level=1){
            $model = M('user');
            $allUsers = $model ->field(array('id','invit_1')) -> where(array(
                                      'id' => array('gt',$id)
                                    )) ->select();
            $subs = $this -> getUnlimitedLayers($allUsers,$id);
            
            $model_issue_log = M('issue_log');
            foreach($subs as $k=>$v){
                if($v['level']==$level){
                    $arr[$k] = $model ->where(array('id'=>$v['id'])) ->find();
                    
                    //左区业绩
                    $sub_users_left = $model ->field(array('id','area')) ->where(array('invit_1'=>$v['id'],'area'=>0)) ->select();
                    if($sub_users_left){
                        foreach($sub_users_left as $kk2=>$vv2){
                            ...
                            $arr[$k]['left_yeji'] += round($left_yeji_decimal,8);
                        }
                    }else{
                        $arr[$k]['left_yeji'] = 0.00;
                    }
                    
                    //右区业绩
                    $sub_users_right = $model ->field(array('id','area')) ->where(array('invit_1'=>$v['id'],'area'=>1)) ->select();
                    if($sub_users_right){
                        foreach($sub_users_right as $kk3=>$vv3){
                            ...
                            $arr[$k]['right_yeji'] += round($right_yeji_decimal,8);
                        }
                    }else{
                        $arr[$k]['right_yeji'] = 0.00;
                    }
         ...
         //分别获取不同层级的元素的
        public function mywd_nextlevel_json($id,$level=1){
          
        //public function mywd_nextlevel_json(){
            //$id = $_GET['id'];
            //$level = 1;
                    ...
                    if($v['level']==$level){
                        ...
                        if($sub_users_left){
                            foreach($sub_users_left as $kk2=>$vv2){
                                $left_yeji_str = $model_issue_log ->field(array('mum')) ->where(array('userid'=>$vv2['id'])) ->find();
                                $left_yeji_decimal = $left_yeji_str ? $left_yeji_str['mum'] : 0.00;
                                $myarr['left_yeji'] += round($left_yeji_decimal,8);
                            }
                        }else{
                            $myarr['left_yeji'] += 0.00;
                        }
                        ...
                        if($sub_users_right){
                            foreach($sub_users_right as $kk3=>$vv3){
                                $right_yeji_str = $model_issue_log ->field(array('mum')) ->where(array('userid'=>$vv3['id'])) ->find();
                                $right_yeji_decimal = $right_yeji_str ? $right_yeji_str['mum'] : 0.00;
                                $myarr['right_yeji'] += round($right_yeji_decimal,8);
                            }
                        }else{
                            $myarr['right_yeji'] = 0.00;
                        }
                    }
         ...
         //分别按层级下级分类
        private function getUnlimitedLayers($cate,$id,$level=0){
            $arr = array();
            foreach ($cate as $k => $v) {
                if ($v['invit_1'] == $id) {
                    $v['level'] = $level + 1;
                    $arr = array_merge($arr,$this->getUnlimitedLayers($cate, $v['id'], $level + 1));
                    $arr[] = $v;
                }
            }
            return $arr;
        ...
        public function apply_show(){
            ...
            foreach($list as $k=>$v){
                $data[$k]['id'] = $v['id'];
                $data[$k]['loginUser'] = $model_user ->field(array('username')) ->where(array('id'=>$v['cid'])) ->find()['username']; //谁发起的申请
                $data[$k]['requestUser'] = $model_user ->field(array('username')) ->where(array('id'=>$v['request_id'])) ->find()['username'];//谁被修改了
                $data[$k]['invit_1'] = $model_user ->field(array('username')) ->where(array('id'=>$v['invit_1'])) ->find()['username'];//被修改指向谁
                $data[$k]['area'] = $v['area'] ? '右区' :  '左区';
        ...
        public function apply_pass(){
            ...
            $model = M('relation_change_apply');
            $res_apply = $model ->where(array('id'=>$id)) ->find();
            //mydump($res_apply);die;
            
            $model_user = M('user');
            $invit_user = $model_user -> field(array('id,username,invit_1,invit_2,invit_3')) ->where(array('id'=>$res_apply['invit_1'])) ->find();
            
            $arr = array(
                   'invit_1' => $res_apply['invit_1'],
                   'invit_2' => $invit_user['invit_1'],
                   'invit_3' => $invit_user['invit_2'],
                   'area' => $res_apply['area'],
                );
            
            $mo = M();
            $mo->execute('set autocommit=0');
            $mo->execute('lock tables movesay_relation_change_apply write,movesay_user write');
            
            $res[] = $mo->table('movesay_user') ->where(array('id'=>$res_apply['invit_1'])) ->setField($arr);
            $res[] = $mo->table('movesay_relation_change_apply') ->where(array('id'=>$id))->setField(array('status'=>1));
