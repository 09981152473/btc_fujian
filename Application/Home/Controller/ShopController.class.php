...
class ShopController extends HomeController
{
	...
	public function view($id)
	{
		...
		if (!$Shop) {
			...
			$this->assign('coin_list', $coin_list);
                ...
                //获取用户总的币数
                /*
                $total_coin_info = M('user_coin') ->where(array('userid'=>userid())) ->find();
                $total_coin = ($total_coin_info['btc'] + $total_coin_info['btcd']) * 0.3;  
	...
	public function buyShop($id, $num, $paypassword, $type, $goods)
	{
		...
		$xuyao = round($mum / $coin_price, 8); //此处有折价(系统原来的)
		...
		//if ($usercoin < $xuyao) {
			...
			//$this->error('可用' . C('coin')[$type]['title'] . '余额不足,总共需要支付' . $xuyao . ' ' . C('coin')[$type]['title']);
                    $this->error('可用' . C('coin')[$type]['title'] . '余额不足,总共需要支付' . $mum . ' ' . C('coin')[$type]['title']);
		}

		$mo = M();
		$mo->execute('set autocommit=0');
		//$mo->execute('lock tables movesay_user_coin write,movesay_shop write,movesay_shop_log write');
                $mo->execute('lock tables movesay_user_coin write,movesay_shop write,movesay_shop_log write,movesay_user_coin_sep write');
		$rs = array();
		$rs[] = $mo->table('movesay_user_coin')->where(array('userid' => userid()))->setDec($type,$mum);//$xuyao
                $rs[] = $mo->table('movesay_user_coin_sep')->where(array('userid' => userid(),'coiname'=>$type,'coinjian'=>$type))->setDec('maket_mum',$mum);//$xuyao
	...
	'num'  => array('exp', 'num-' . $num)
		...
		$rs[] = $mo->table('movesay_shop_log')->add(array('userid' => userid(), 'shopid' => $Shop['id'], 'price' => $Shop['price'], 'coinname' => $type, 'num' => $num, 'mum' => $mum, 'addr' => $my_goods['truename'] . '|' . $my_goods['moble'] . '|' . $my_goods['addr'], 'addtime' => time(), 'status' => 0));//'xuyao' => $xuyao,
		if (check_arr($rs)) {
			$mo->execute('commit');
