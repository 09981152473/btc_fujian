...
/* [31m * TODO SEPARATE[0m */
		...
		//$user_coin = M('UserCoin')->where(array('userid' => userid()))->find();//原来的？？？？？？？
                
                //自己的修改，改变数据判断方向
                $user_coin_sep_info = M('user_coin_sep') ->where(array(
                                  'userid' => userid(),
                                  'coiname' => $xnb,
                                  'coinjian' => $xnb
                             )) ->find();
                $user_coin = $user_coin_sep_info['trade_mum'];
                //自己修改的结束
                
                //echo json_encode(array('status'=>1,'xnb'=>$xnb));
                //echo json_encode(array('status'=>1,'rmb'=>$rmb));
                
           
		if ($type == 1) { //买入:使用人民币买入，不限制
...
/* [31m * TODO SEPARATE[0m */
				//$bili_user = round($user_coin[$xnb] + $user_coin[$xnb . 'd'], C('market')[$market]['round']);
                                $bili_user = round($user_coin, C('market')[$market]['round']);
...
/* [31m * TODO SEPARATE[0m */
	...
	'market'  => array('like', '%' . $xnb . '%')
		...
		//$mo->execute('lock tables movesay_trade write ,movesay_user_coin write ,movesay_finance write');
                $mo->execute('lock tables movesay_trade write ,movesay_user_coin write ,movesay_finance write ,movesay_user_coin_sep write');
		$rs = array();
		...
		if ($type == 1) {
			...
			$rs[] = $mo->table('movesay_user_coin')->where(array('userid' => userid()))->setInc($rmb . 'd', $mum);
                        ...
                        $rs[] = $mo->table('movesay_user_coin_sep')->where(array(
                                     'userid' => userid(),
                                     'coiname' => $xnb,
                                     'coinjian' => $xnb.'d',
                                )) ->setDec('trade_mum',$mum);
