...
class LoginController extends HomeController
{
	...
	public function upregister($username, $password, $repassword, $verify, $invit)
	{
		...
		for ($invit_3 = 0; ; ) {
			...
			if (!M('User')->where(array('invit' => $tradeno))->find()) {
                ...
                //设置区域
                $invited =  M('user') ->where(array('invit_1'=> $invituser['id'])) ->select();
                if(count($invited)){
                    $area = rand(0,1);
                }else{
                    $area = 0; 
	...
	public function findpaypwd()
	{
        ...
        public function checkInvitCode(){
            $invitCode = trim($_GET['invitCode']);
            
            $res = M('user') ->where(array('invit'=>$invitCode)) ->select();
            if($res){
                echo json_encode(array('status' => 1, 'des'=>'valid'));
            }else{
                echo json_encode(array('status'=>0,'des'=>'invalid'));
            }
