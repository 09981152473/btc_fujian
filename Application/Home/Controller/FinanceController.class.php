...
class FinanceController extends HomeController
{
	...
	public function mywd()
	{
		...
		$Page = new \Home\Common\Ext\PageExt($count, 10);
		$show = $Page->show();
		$list = $Model->where($where)->order('id asc')->field('id,username,moble,addtime,invit_1,area')->limit($Page->firstRow . ',' . $Page->listRows)->select();


		foreach ($list as $k1 => $v1) {
			$list[$k1]['invits'] = M('User')->where(array('invit_1' => $v1['id']))->order('id asc')->field('id,username,moble,addtime,invit_1,area')->select();
			$list[$k1]['invitss'] = count($list[$k1]['invits']);
                        
                        foreach ($list[$k1]['invits'] as $k2 => $v2) {
				$list[$k1]['invits'][$k2]['invits'] = $Model->where(array('invit_1' => $v2['id']))->order('id asc')->field('id,username,moble,addtime,invit_1,area')->select();
				$list[$k1]['invits'][$k2]['invitss'] = count($list[$k1]['invits'][$k2]['invits']);
        ...
        public function mywd()
	{
		...
		$where['invit_1'] = userid();
		$Model = M('User');
		$list = $Model->where($where)->order('id asc')->field('id,username,moble,addtime,invit_1,area')->select();
                
                $model_issue_log = M('issue_log');
                
                //左区业绩
                    $sub_users_left = $Model ->field(array('id','area')) ->where(array('invit_1'=>userid(),'area'=>0)) ->select();
                    if($sub_users_left){
                    ...
                    //右区业绩
                    $sub_users_right = $Model ->field(array('id','area')) ->where(array('invit_1'=>userid(),'area'=>1)) ->select();
                    if($sub_users_right){
        ...
        //分别获取不同层级的元素的
        public function mywd_ajax($id,$level=1){
            $model = M('user');
            ...
            foreach($subs as $k=>$v){
                if($v['level']==$level){
                    $arr[$k] = $model ->field(array('id,username,addtime,moble,area')) ->where(array('id'=>$v['id'])) ->find();
                    $addtime = date('Y-m-d H:i:s',$arr[$k]['addtime']);
                    $arr[$k]['addtime'] = $addtime;
                }
            }
	...
	public function check()
	{
	    ...
	    check_server();
            ...
            $id = $_GET['id'];
            $model = M('user');
            $user = $model ->field(array('id,username,addtime,moble,area,invit_1,invit_2,invit_3')) ->where(array('id'=>$id)) ->find();
            $this ->assign('user',$user);
            
            //下拉列表：指向同级的会员
            $sameLayerUsers = $model ->field(array('id,username,area')) ->where(array(
        ...
        public function showUsers(){
            ...
            $model = M('user');
            $user = $model ->field(array('id,username,area,invit_1,invit_2,invit_3')) ->where(array('id'=>$id)) ->find();
